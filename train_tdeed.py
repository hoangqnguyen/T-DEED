...
def get_args():
    ...
    parser.add_argument("--model", type=str, default="imrcvolley_small")
    parser.add_argument(
...
def update_args(args, config):
    ...
    args.save_dir = (
        config["save_dir"] + "/" + args.model
    )  # + '-' + str(args.seed) -> in case multiple seeds
...
def main(args):
    ...
    classes, pretrain_classes, train_data, val_data, val_data_frames = get_datasets(
        args
    )
    ...
    if args.store_mode == "store":
        ...
        sys.exit(
            "Datasets have correctly been stored! Stop training here and rerun with load mode."
        )
    else:
    ...
    if not args.only_test:
        ...
        for epoch in range(epoch, num_epochs):
            ...
            if (
                ...
                & (args.criterion == "map")
                & (epoch >= args.start_val_epoch)
            ):
                val_loss, map_labels, map_preds = model.epoch(
                    val_loader, acc_grad_iter=args.acc_grad_iter, valMAP=True
                )
            else:
            ...
            print(
                "Time train: "
                + str(int(time_train // 60))
                + "min "
                + str(np.round(time_train % 60, 2))
                + "sec"
            )
            print(
                "Time val: "
                ...
                wandb.log(
                    {
                        ...
                        "times/time_train": time_train,
                        "times/time_val": time_val,
                        "times/time_map": time_map,
                    }
                )
            else:
                wandb.log(
            ...
            if args.dataset == "soccernet":
                ...
                for j in range(len(classes)):
                    wandb.log(
                        {
                            "test/classes/mAP@"
                            + list(classes.keys())[j]: results["a_mAP_per_class"][j]
                            * 100
                        }
                    )
                ...
                for j in range(len(classes)):
                    wandb.log(
                        {
                            "test/classes/mAP_loose@"
                            + list(classes.keys())[j]: results_loose["a_mAP_per_class"][
                                j
                            ]
                            * 100
                        }
            ...
            if args.dataset == "soccernetball":
                ...
                for i in range(len(results["a_mAP_per_class"])):
                    print(
                        ...
                        + list(classes.keys())[i]
                        + ": "
                        + str(np.round(results["a_mAP_per_class"][i] * 100, 2))
                    )
