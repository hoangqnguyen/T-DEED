        ...
        for video in tqdm(self._labels):
            ...
            if self._dataset == "soccernet":
                ...
                labels_file = load_json(
                    os.path.join(
                        LABELS_SN_PATH,
                        "/".join(video["video"].split("/")[:-1]) + "/Labels-v2.json",
                    )
                )["annotations"]
            elif self._dataset == "soccernetball":
                video_half = 1
                labels_file = load_json(
                    os.path.join(LABELS_SNB_PATH, video["video"] + "/Labels-ball.json")
                )["annotations"]
            else:
            ...
            for base_idx in range(
                ...
                max(
                    0,
                    video_len - 1 + (2 * self._pad_len - self._clip_len) * self._stride,
                ),
                self._overlap,
            ):
                ...
                for event in labels_file:
                    ...
                    if (self._dataset == "soccernet") | (
                        ...
                        if event_half == video_half:
                            event_frame = int(
                                int(event["position"]) / 1000 * FPS_SN
                            )  # miliseconds to frames
                            label_idx = (event_frame - base_idx) // self._stride

                            if self._radi_displacement >= 0:
                                if (
                                    label_idx >= -self._radi_displacement
                                    and label_idx
                                    < self._clip_len + self._radi_displacement
                                ):
                    ...
                    # For other datasets
                        ...
                        if self._radi_displacement >= 0:
                            if (
                                label_idx >= -self._radi_displacement
                                and label_idx < self._clip_len + self._radi_displacement
                            ):
                                label = self._class_dict[event["label"]]
                                for i in range(
                                    max(0, label_idx - self._radi_displacement),
...
class FrameReader:
    ...
    def read_frame(self, frame_path):
        img = torchvision.io.read_image(
            frame_path
        )  # .float() / 255 -> into model normalization / augmentations
        return img
        ...
        for frame_num in range(start, end, stride):
            ...
            if self.dataset == "finediving":
                ...
                frame_path = os.path.join(
                    self._frame_dir, video_name, "frame" + str(frame) + ".jpg"
                )
            ...
            elif self.dataset == "imrcvolley":
                frame = frame_num
                frame_path = os.path.join(path, f"{frame:06d}.jpg")
                base_path = path
                ndigits = -1
        ...
        if ndigits == -1:
            ...
            if self.dataset != "imrcvolley":
                ...
                _ = [
                    ret.append(
                        self.read_frame(
                            os.path.join(base_path, f"{start + j * stride:06d}.jpg")
                        )
                    )
                    for j in range(length - pad_start - pad_end)
                ]
            ...
            _ = [
                ret.append(
                    self.read_frame(
                        path + str(start + j * stride).zfill(ndigits) + ".jpg"
                    )
                )
                for j in range(length - pad_start - pad_end)
            ]
    ...
    def __init__(
        ...
        for l in self._labels:
            ...
            for i in range(
                ...
                max(
                    0, l["num_frames"] - (overlap_len * stride)
    ...
    def get_labels(self, video):
        ...
        if self._dataset == "soccernet":
            labels_file = load_json(
                os.path.join(
                    LABELS_SN_PATH,
                    "/".join(meta["video"].split("/")[:-1]) + "/Labels-v2.json",
                )
            )["annotations"]
            labels_half = int(video[-1])
        elif self._dataset == "soccernetball":
            labels_file = load_json(
                os.path.join(LABELS_SNB_PATH, meta["video"] + "/Labels-ball.json")
            )["annotations"]
            labels_half = 1
        else:
        ...
        if (self._dataset == "soccernet") | (self._dataset == "soccernetball"):
            return sorted(
                [
                    (
                        v["video"],
                        math.ceil(v["num_frames"] / self._stride),
                        FPS_SN / self._stride,
                    )
                ...
                if self._dataset == "soccernet":
                    ...
                    for e in labels_file:
                        ...
                        if half == int(x_copy["video"][-1]):
                            e["frame"] = (
                                int(int(e["position"]) / 1000 * FPS_SN) // self._stride
                            )
                    ...
                    labels_file = load_json(
                        os.path.join(
                            LABELS_SNB_PATH, x_copy["video"] + "/Labels-ball.json"
                        )
                    )["annotations"]
                    for e in labels_file:
                        e["frame"] = (
...
class FrameReaderVideo:
    ...
    def read_frame(self, frame_path):
        img = torchvision.io.read_image(
            frame_path
        )  # / 255 -> modified for ActionSpotVideoDataset (to be compatible with train reading without / 255)
        return img

    def load_frames(
        self, video_name, start, end, pad=False, stride=1, source_info=None
    ):
        ret = []
        n_pad_start = 0
        n_pad_end = 0
        ...
        for frame_num in range(start, end, stride):
            ...
            if self._dataset == "finediving":
                frame_path = os.path.join(
                    path, str(frame0 + frame_num).zfill(ndigits) + ".jpg"
                )
                ...
                frame_path = os.path.join(
                    path, "frame" + str(frame0 + frame_num) + ".jpg"
                )
